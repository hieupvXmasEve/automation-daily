...
from .streamlit_workflows import run_compare_uploads, run_extract_upload, run_tiktok_pdf_audit_upload
...
def render_compare_tab() -> None:
    with st.form("compare-form"):
        ...
        only_statuses = st.multiselect("Only export statuses", ["matched", "mismatch", "missing-in-excel", "missing-in-pdf"])
    ...
    if submitted:
        if excel_file is None or pdf_file is None:
            ...
            st.session_state[COMPARE_RESULT_KEY] = _run_action("Running compare workflow", run_compare_uploads, excel_file, pdf_file, formats, only_statuses)
...
def render_extract_tab() -> None:
    ...
    if submitted:
        if excel_file is None:
            ...
            st.session_state[EXTRACT_RESULT_KEY] = _run_action("Building item export", run_extract_upload, excel_file, export_format)
    ...
    if result:
        ...
        st.download_button(label=f"Download {result['file_name']}", data=result["data"], file_name=result["file_name"], mime=result["mime"], use_container_width=True)
...
def render_tiktok_audit_tab() -> None:
    ...
    if submitted:
        if pdf_file is None:
            ...
            st.session_state[TIKTOK_AUDIT_RESULT_KEY] = _run_action("Parsing TikTok PDF and grouping rows by order id", run_tiktok_pdf_audit_upload, pdf_file, export_format)
    result = st.session_state.get(TIKTOK_AUDIT_RESULT_KEY)
    if result:
        _render_tiktok_audit_result(result)


def _run_action(progress_message: str, action, *args):
    progress_text = st.empty()
    progress_bar = st.progress(0)
    try:
        progress_text.info(progress_message)
...
def _render_compare_result(result: dict[str, object]) -> None:
    ...
    with downloads_tab:
        for file_name, payload in result["downloads"].items():
            st.download_button(label=f"Download {file_name}", data=payload["data"], file_name=file_name, mime=payload["mime"], use_container_width=True)


def _render_tiktok_audit_result(result: dict[str, object]) -> None:
    summary = result["summary"]
    top_metrics = st.columns(3)
